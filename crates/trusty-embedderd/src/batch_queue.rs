...
//!
//! Why: batching collapses the per-call ONNX overhead into one larger matrix
//! operation, dramatically improving throughput under concurrent load. This is
//! especially relevant when many callers (e.g. trusty-memory recall queries)
//! arrive within the same 10 ms window. Ported verbatim from the retired
//! `trusty-embed-daemon` crate (issue #164 Step 2).
//!
...
///
/// Why: 512 covers a worst-case burst (50 concurrent callers × ~10 in
/// flight each) with headroom. Above that we backpressure the writer, which
/// surfaces overload to callers instead of silently queuing thousands of texts.
...
///
/// Why: the server hands one of these to every handler so that per-request
/// code can submit texts and await their embeddings without ever touching the
/// embedder directly.
///
...
impl BatchQueue {
    ...
    ///
    /// What: creates the bounded mpsc channel, spawns `batch_worker` with the
    /// supplied config, returns the sender wrapped in `BatchQueue`.
    ///
    ...
    #[allow(dead_code)] // part of the documented queue API; not yet wired to a handler
...
mod tests {
    ...
    async fn batch_queue_collapses_concurrent_requests() {
        // Why: verify that concurrent callers each receive a correct, distinct
        // embedding vector from the BatchQueue despite going through one worker.
        // What: fire 16 concurrent embed_one calls and assert correct dimensionality.
        // Test: this test.
    ...
    async fn embed_many_preserves_order() {
        // Why: order preservation is the primary correctness property; callers
        // rely on results[i] corresponding to texts[i].
        // What: embed three distinct texts and compare against direct mock invocation.
        // Test: this test.
        let embedder: Arc<dyn Embedder> = Arc::new(MockEmbedder::new(EMBED_DIM));
    ...
    async fn embed_many_empty_returns_empty() {
        // Why: callers must be able to pass an empty batch without error.
        // What: call embed_many([]) and assert the result is empty.
        // Test: this test.
        let embedder: Arc<dyn Embedder> = Arc::new(MockEmbedder::new(EMBED_DIM));
        let queue = BatchQueue::new(embedder, BatchConfig::default());
