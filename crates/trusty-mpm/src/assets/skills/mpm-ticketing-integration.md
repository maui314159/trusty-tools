...
**GitHub is always the default ticketing system.** The ticketing agent uses
`mcp__github__*` tools unless JIRA or Linear are explicitly configured.

Routing order (evaluated by ticketing agent):
...
- ticketing agent MUST NOT auto-create a new issue.
- ticketing agent MUST ask: "I didn't find an existing issue for [topic]. Should I create one on GitHub, or did you mean a different issue?"
- Auto-create only when user explicitly says "create a ticket/issue for X."

## Detection Rules

PM detects ticket context from:
- Ticket ID patterns: `PROJ-123`, `#123`, `MPM-456`
...
Task:
  ...
  acceptance_criteria:
    ...
    - Add comment: "Work started by trusty-mpm"
...
- Still create local docs as backup in `docs/`
- All agent delegations include ticket context

### When NO Ticket Context

- All documentation goes to `docs/research/`
