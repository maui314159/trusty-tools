//! open-mpm desktop chat (Tauri 2).
//!
//! Why: Gives users a native chat UI for talking to the CTRL controller and
//! project-scoped PMs without hand-running `open-mpm --task '…'` at the
//! command line. The Rust side here only does three things: (1) spawn the
//! `open-mpm --api` sidecar on startup so the REST server is reachable,
...
/// Shared handle to the spawned `open-mpm --api` sidecar.
///
...
/// Spawn `open-mpm --api --port <port>` if not already running.
///
...
/// spawns `open-mpm --api --port <port>` (resolved relative to $PATH first,
...
async fn ensure_api_server(port: u16, state: State<'_, SharedApi>) -> Result<(), String> {
    ...
    if let Some(ref mut existing) = *guard {
        ...
        match existing.try_wait() {
            Ok(Some(_status)) => {
                ...
                tracing::warn!(port, "open-mpm sidecar exited; respawning");
                *guard = None;
            }
            Ok(None) => {
    ...
    let binary = resolve_open_mpm_binary();
    ...
    // would result in attempts to create `/.open-mpm/state` (EROFS). Pass the
    // compile-time-known open-mpm project root via OPEN_MPM_PROJECT_DIR so the
    ...
    // sidecar was crashing on `create_dir_all("/.open-mpm/state")` before
    // binding the HTTP listener.
    // What: Set OPEN_MPM_PROJECT_DIR to the open-mpm repo root derived from
    ...
    let child = tokio::process::Command::new(&binary)
        ...
        .arg("--port")
        .arg(port.to_string())
        .env("OPEN_MPM_PROJECT_DIR", &project_root)
...
/// Find the `open-mpm` binary. Prefers `$PATH`, then the sibling Cargo
/// workspace's debug/release target so `cargo run` in this repo's root
/// doesn't require a global install.
fn resolve_open_mpm_binary() -> std::path::PathBuf {
    // 1. $PATH (works in dev / CLI contexts)
    if let Ok(path) = which("open-mpm") {
        return path;
    }
    ...
    if let Ok(home) = std::env::var("HOME") {
        ...
        for candidate in [
            home.join(".cargo/bin/open-mpm"),
            home.join(".local/bin/open-mpm"),
        ] {
    ...
    // 3. Sibling Cargo workspace target dir (ui/src-tauri → open-mpm root).
    let manifest = std::path::Path::new(env!("CARGO_MANIFEST_DIR"));
    if let Some(root) = manifest.ancestors().nth(2) {
        for profile in ["release", "debug"] {
            let candidate = root.join("target").join(profile).join("open-mpm");
            if candidate.is_file() {
                return candidate;
            }
        }
    ...
    std::path::PathBuf::from("open-mpm")
}
...
/// Test: Run `open-mpm --api` manually, call this command with `content=
    ...
    tauri::Builder::default()
        ...
        .on_window_event(|window, event| {
            if let tauri::WindowEvent::Destroyed = event {
                // Reap the spawned open-mpm sidecar on window close so we
