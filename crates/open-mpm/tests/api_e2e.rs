//! End-to-end tests for the open-mpm HTTP API (#183).
//!
...
//! over the wire we need tests that hit a real `open-mpm --api` process.
...
async fn test_ctrl_connects_to_test_project() {
    ...
    let task = format!(
        "you are connected to a project at {}. List the agent names you find under .open-mpm/agents/.",
