...
//!
//! Why: Multiple open-mpm projects running concurrently need a lightweight
...
//! What: `MessageBus` owns a `UnixListener` at `~/.open-mpm/sockets/<id>.sock`
...
/// `~/.open-mpm/sockets/*.sock` and connection is a simple path connect.
...
impl MessageBus {
    ...
    /// What: Returns `~/.open-mpm/sockets/<project_id>.sock`.
    ...
    /// What: Reads `~/.open-mpm/sockets/`, returns the stem of every `*.sock`
    ...
    pub async fn list_running() -> Result<Vec<String>> {
        let home = dirs::home_dir().ok_or_else(|| anyhow::anyhow!("no home dir"))?;
        let sockets_dir = home.join(".open-mpm").join("sockets");
