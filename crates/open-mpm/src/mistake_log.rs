...
pub struct MistakeRecord {
    ...
    pub ts: String,
    /// Session identifier (matches OPEN_MPM_RUN_ID or build label).
...
impl MistakeLog {
    ...
    /// What: Writes to
    /// `<project>/.open-mpm/state/mistakes/<session_id>.jsonl` and
    /// `~/.open-mpm/sessions/mistakes.jsonl`. Both files are created if
    /// missing.
    ...
    /// a focused report.
    /// What: Reads `<project>/.open-mpm/state/mistakes/<session_id>.jsonl`,
    ...
    /// What: Reads `~/.open-mpm/sessions/mistakes.jsonl` and returns the
...
/// Resolve `<project>/.open-mpm/state/mistakes/<session_id>.jsonl`.
...
/// Resolve `~/.open-mpm/sessions/mistakes.jsonl`.
