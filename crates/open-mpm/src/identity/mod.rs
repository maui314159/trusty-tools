...
//! Session, Agent). Construction from `OPEN_MPM_CALLER` env vars happens
...
/// What: Four variables — `OPEN_MPM_CALLER` selects the variant, the rest
/// carry the IDs each variant needs.
/// Test: `caller_identity_from_env_*` round-trips for each variant.
pub const ENV_CALLER: &str = "OPEN_MPM_CALLER";
pub const ENV_SESSION_ID: &str = "OPEN_MPM_SESSION_ID";
pub const ENV_PROJECT_ID: &str = "OPEN_MPM_PROJECT_ID";
pub const ENV_AGENT_ID: &str = "OPEN_MPM_AGENT_ID";
...
impl CallerIdentity {
    ...
    /// per agent run).
    /// What: Reads `OPEN_MPM_CALLER`. For `pm`/`agent` requires
    /// `OPEN_MPM_SESSION_ID` + `OPEN_MPM_PROJECT_ID` (and `OPEN_MPM_AGENT_ID`
    ...
    pub fn from_env() -> Option<Self> {
        let caller = std::env::var(ENV_CALLER).ok()?.to_lowercase();
        match caller.as_str() {
            "ctrl" => Some(Self::Ctrl),
            "pm" => {
