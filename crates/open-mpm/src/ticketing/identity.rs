...
//! constant env juggling. Named identities in `~/.open-mpm/config.toml` let
...
/// `[github]` section of `~/.open-mpm/config.toml` (#243).
///
...
mod tests {
    ...
    fn unique_var(prefix: &str) -> String {
        format!("OPEN_MPM_TEST_{}_{}", prefix, uuid::Uuid::new_v4().simple())
    }
