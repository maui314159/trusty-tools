...
/// What: Returns `open-mpm vX.Y.Z (<git-hash>)`.
/// Test: `version_string_contains_version` checks both substrings render.
pub fn version_string() -> String {
    format!("open-mpm v{VERSION} ({GIT_HASH})")
}

/// On-disk shape of `.open-mpm/state/build.json`.
///
...
/// Test: `display_string` returns `open-mpm vX.Y.Z build #N`.
#[derive(Debug, Clone)]
...
impl BuildInfo {
    ...
    /// What: Uses `$CWD/.open-mpm/state/build.json`. See
    ...
    pub async fn load_and_increment() -> Result<Self> {
        ...
        let dir = cwd.join(".open-mpm").join("state");
    ...
    /// mutating the real `.open-mpm/` in the repo root.
    ...
    /// What: `open-mpm vX.Y.Z build #N`.
    /// Test: Asserted directly in unit tests.
    pub fn display_string(&self) -> String {
        format!("open-mpm v{} build #{}", self.version, self.build)
    }
}
...
mod tests {
    ...
    async fn display_string_format() {
        ...
        assert_eq!(info.display_string(), "open-mpm v0.1.0 build #42");
    }

    #[test]
    fn version_string_contains_version() {
        let s = version_string();
        assert!(s.contains("open-mpm v"));
    ...
    async fn creates_missing_parent_dir() {
        ...
        let nested = root.path().join("deep").join(".open-mpm");
