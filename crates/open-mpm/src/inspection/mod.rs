...
/// Execute `open-mpm inspect --task <text> [--dry-run]`.
///
...
pub async fn run_inspect_subcommand(args: &[String]) -> Result<()> {
    ...
    while i < args.len() {
        match args[i].as_str() {
            ...
            other => {
                bail!(
                    "open-mpm inspect: unknown argument '{other}'. \
    ...
    let task_text = match (task, task_file) {
        ...
        (None, None) => bail!("open-mpm inspect requires --task <text> or --task-file <path>"),
    };
...
pub async fn run_inspect_live(
    ...
    let mut pm_cfg = AgentConfig::by_name("pm")
        .context("failed to load pm agent config — is .open-mpm/agents/pm.toml present?")?;
...
mod tests {
    ...
    /// Path to the repo-bundled `.open-mpm` directory (agents + skills).
    fn bundled_config_dir() -> PathBuf {
        PathBuf::from(env!("CARGO_MANIFEST_DIR")).join(".open-mpm")
    }
    ...
    // entry in `.open-mpm/tasks/harness-test-suite.toml`.
