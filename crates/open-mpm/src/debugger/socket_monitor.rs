//! Liveness checker for the open-mpm ctrl socket.
//!
...
//! What: `SocketMonitor` derives the canonical `~/.open-mpm/sockets/<id>.ctrl.sock`
...
//! behaviour is observed in manual integration runs of `open-mpm debug`.
...
impl SocketMonitor {
    ...
    /// What: Builds `~/.open-mpm/sockets/<sanitized-basename>.ctrl.sock`.
...
mod tests {
    ...
    fn socket_monitor_path_matches_canonical() {
        let m = SocketMonitor::new(Path::new("/tmp/open-mpm"));
        let s = m.socket_path().to_string_lossy().to_string();
        assert!(s.ends_with("/.open-mpm/sockets/open-mpm.ctrl.sock"), "{s}");
    }
    ...
    fn socket_monitor_dead_when_no_listener() {
        ...
        let m = SocketMonitor::new(Path::new("/tmp/__open_mpm_ghost_dir__"));
