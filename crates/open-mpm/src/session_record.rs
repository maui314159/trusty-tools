//! Cross-project session record store at `~/.open-mpm/sessions/runs.jsonl`.
//!
...
/// One row in `~/.open-mpm/sessions/runs.jsonl`.
///
...
/// Resolve `~/.open-mpm/sessions/runs.jsonl`.
///
...
pub fn runs_path() -> Result<PathBuf> {
    ...
    Ok(home.join(".open-mpm").join("sessions").join("runs.jsonl"))
}
...
/// Append one record as a JSON line to `~/.open-mpm/sessions/runs.jsonl`.
///
...
pub async fn append_to(path: &Path, record: &SessionRecord) -> Result<()> {
    ...
    // open-mpm processes (API server + CLI + GUI) cannot interleave bytes
