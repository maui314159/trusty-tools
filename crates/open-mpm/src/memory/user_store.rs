//! User-scoped memory store at `~/.open-mpm/memory/`.
//!
...
//! `~/.open-mpm/memory/` directory. The `insert` / `search_relevant`
...
/// User-scoped memory store backed by redb + usearch at `~/.open-mpm/memory/`.
///
...
impl UserMemoryStore {
    ...
    pub async fn open() -> Result<Self> {
        ...
        let path = home.join(".open-mpm").join("memory");
