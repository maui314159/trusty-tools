...
//! User vs. Project scope, loadable from `.open-mpm/config.toml`. The render
...
/// from `.open-mpm/config.toml` keeps the UI tunable.
...
/// Top-level wrapper for `.open-mpm/config.toml` parsing.
...
impl StatuslineConfig {
    /// Load from `<project>/.open-mpm/config.toml` if present, otherwise default.
    ///
    ...
    pub fn load(project_dir: &Path) -> Self {
        let path = project_dir.join(".open-mpm").join("config.toml");
...
mod tests {
    ...
    fn statusline_config_load_parses_user_overrides() {
        let tmp = tempfile::tempdir().unwrap();
        std::fs::create_dir_all(tmp.path().join(".open-mpm")).unwrap();
        let toml = r#"
[statusline]
user_segments = ["scope", "elapsed"]
project_segments = ["model", "git"]
"#;
        std::fs::write(tmp.path().join(".open-mpm/config.toml"), toml).unwrap();
        let cfg = StatuslineConfig::load(tmp.path());
    ...
    fn render_statusline_project_scope_shows_segments() {
        ...
        app.working_dir = "/Users/x/projects/open-mpm".to_string();
        app.git_branch = Some("main".to_string());
        app.git_dirty = true;
        let s = render_statusline(&app);
        assert_eq!(s, "openrouter | claude-sonnet-4-6 | open-mpm | main*");
    }
