//! Stable plugin API surface shared between `open-mpm` and external agent crates.
//!
//! Why: The original design placed `ToolExecutor` / `AgentPlugin` / `ToolResult`
//!      inside `open-mpm`'s `lib.rs`. That created a hard cargo dependency
//!      cycle (open-mpm → cto-assistant → open-mpm), because external agent
//!      crates need the trait to implement it AND `open-mpm` needs the agent
...
//!      crate breaks the cycle: both `open-mpm` and every agent crate depend
//!      on `open-mpm-agent-api`, but never on each other through the lib.
//! What: Re-defines the previously open-mpm-internal types — `ToolExecutor`
//!       trait, `ToolResult` enum, `ToolExecutionTier` enum, `ServiceTier`
//!       enum (RBAC tiers), and `AgentPlugin` struct — as the public surface.
//!       `open-mpm` re-exports them via `open_mpm::agent_api` for source-level
//!       compatibility with the existing call sites in `crates/open-mpm/src/**`.
//! Test: Compile-tested transitively via `crates/cto-assistant` (downstream
//!       agent) and `crates/open-mpm` (host).
...
///       returns the success string. Exercised across `open-mpm/tools/**`.
#[derive(Debug)]
pub enum ToolResult {
    Success(String),
...
impl ToolResult {
    ...
    /// Test: Used by `is_fatal` tests in open-mpm.
...
/// Two-tier tool execution model (open-mpm #447).
///
...
///       `open-mpm`'s `tools/always_on::build_live_context_*`.
...
/// RBAC service tier (open-mpm #445).
///
...
///      (not in `open-mpm/rbac`) because the `ToolExecutor::restricted_tiers`
...
/// Test: `open-mpm/rbac` covers serde + ordering; `open-mpm/tools/mod::dispatch_for_user_*`
...
///      Living in `open-mpm-agent-api` (not in `open-mpm`) so external agent
...
///       the loop.
/// Test: See unit tests in `open-mpm/tools/mod.rs` for `ToolRegistry`.
#[async_trait]
pub trait ToolExecutor: Send + Sync {
    ...
    /// Test: `open-mpm/tools/mod::filter_tools_for_user_*`.
    ...
    /// Test: `open-mpm/tools/always_on::build_live_context_*`.
...
///
/// Why: Replaces hard-coded persona-to-tool branches in `open-mpm`'s
...
///      `open-mpm`'s `main.rs`; ctrl never needs to learn their names.
///      Lives here (not in `open-mpm`) so agent crates can construct one
