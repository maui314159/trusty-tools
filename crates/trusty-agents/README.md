...
the trusty-agents/trusty-mpm boundary (including the proposed rename).

A lightweight, composable AI agent orchestration harness in Rust.

`trusty-agents` runs a **PM (Project Manager) orchestrator** that delegates tasks
...
```bash
# Clone and configure
git clone <repo-url> trusty-agents
cd trusty-agents
...
| `--workflow <name>`          | Run `.trusty-agents/workflows/<name>.json` phase by phase.       |
...
| `--version`, `-V`            | Print `tagent vX.Y.Z (<git-hash>)` and exit.              |
...
### Skill Markdown (`.trusty-agents/skills/*.md`)
...
- Session histories persist under `.trusty-agents/sessions/<run_id>/` and are
...
| `TAGENT_CONFIG_DIR`       | Override for `.trusty-agents/agents/` lookup (defaults to repo-relative). |
| `TAGENT_OUT_DIR`          | Default per-run output root when `--out-dir` is omitted.             |
| `TAGENT_RUN_ID`           | Set automatically; inherited by sub-agents to share a `run_id`.      |
...
`version_string()` function renders `tagent vX.Y.Z (<sha>)` and is shown in
...
repo-level `docs/trusty-agents/` tree — not in this crate. Start at
[`docs/trusty-agents/README.md`](../../docs/trusty-agents/README.md) (the master index),
or jump straight to:

- [`docs/trusty-agents/spec/`](../../docs/trusty-agents/spec/) — canonical PRD, ARCHITECTURE, COMPONENTS.
