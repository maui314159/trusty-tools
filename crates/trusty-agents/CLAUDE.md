...
PM Orchestrator (main process)
    |  reads: .trusty-agents/agents/pm.toml
...
Sub-Agent Process (spawned via tokio::process::Command)
    |  reads: .trusty-agents/agents/<name>.toml
...
├── .trusty-agents/              # Bundled config (committed) + runtime state
...
config; it has been folded into `.trusty-agents/` (v0.1.25) so the harness's own
config follows the same layout users adopt in their projects. Runtime state
lives under `.trusty-agents/state/` (gitignored) to keep it out of commits.

Note: Crate documentation (research, design, spec, user, developer docs) now
lives in the repo-level tree at `docs/trusty-agents/` rather than in-crate. See
`docs/trusty-agents/README.md` for the index.
...
Reference: `docs/trusty-agents/research/bake-off-challenges.md`

## Research

All research docs live in the repo-level tree at `docs/trusty-agents/research/`
(indexed by `docs/trusty-agents/research/README.md`). Highlights:
