...
//!
//! What: `ProjectInitializer` takes a project root + the `.trusty-agents/state/`
...
//! disk; else it runs `scan_project`, writes `.trusty-agents/state/project-index.md`
//! and `.trusty-agents/state/initialized`, and returns the fresh `InitContext`.
//!
...
/// Persistent summary stored in `.trusty-agents/initialized`.
///
...
/// dir (`.trusty-agents/`) and the project root separate so tests can point them
/// both at tempdirs.
...
impl ProjectInitializer {
    ...
    /// What: Reads `.trusty-agents/initialized`; if fresh (< `MARKER_TTL_HOURS`),
    ...
    pub async fn initialize_if_needed(&self) -> Result<InitContext> {
        tokio::fs::create_dir_all(&self.agent_dir)
            ...
            .with_context(|| {
                format!(
                    "failed to create trusty-agents dir: {}",
    ...
    async fn read_cached_context(&self) -> Result<Option<InitContext>> {
        let marker_path = self.agent_dir.join("initialized");
        let index_path = self.agent_dir.join("project-index.md");
    ...
    /// Emit the index Markdown to `.trusty-agents/project-index.md`.
    pub async fn write_index(&self, index: &ProjectIndex) -> Result<()> {
        let path = self.agent_dir.join("project-index.md");
