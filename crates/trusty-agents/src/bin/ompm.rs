//! ompm — thin CLI client for the trusty-agents HTTP API (#151 phase-3).
//!
//! Why: Separates the user-facing ergonomic surface from the orchestrator
//! binary. Users run a long-lived server (`trusty-agents --serve`) and interact
...
#[command(
    name = "ompm",
    about = "Thin client for the trusty-agents HTTP API",
...
enum OmpmCommand {
    ...
    /// `/help`, etc.) that the `trusty-agents` REPL exposes, without spinning up
    ...
    /// status`) and execs `trusty-agents /service status`.
    Slash {
...
///
/// Why: avoids coupling the thin client to the full `trusty-agents` crate. We
...
async fn cmd_slash(rest: &[String]) -> anyhow::Result<()> {
    ...
    let status = std::process::Command::new("tagent")
        .args(rest)
        .status()
        .map_err(|e| anyhow::anyhow!("failed to spawn `tagent`: {e}"))?;
    if !status.success() {
...
fn print_usage() {
    eprintln!(
        "ompm — thin client for the trusty-agents HTTP API\n\
\n\
Usage:\n\
...
Slash commands:\n\
  ...
  ompm slash /<cmd> [args]    — passthrough to `tagent /<cmd>` (#344)\n\
\n\
Agent prefix:\n\
