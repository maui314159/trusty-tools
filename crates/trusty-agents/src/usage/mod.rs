...
//!
//! Why: trusty-agents dispatches LLM calls through several paths (claude CLI,
...
//! dispatch to `.trusty-agents/state/usage.jsonl` so operators can audit cost
...
/// `<project_dir>/.trusty-agents/state/usage.jsonl`.
///
...
/// What: Best-effort `mkdir -p` of `.trusty-agents/state`, then opens the file
...
///
/// Why: The usage log lives under `.trusty-agents/state/`, which is per-project.
/// Most call sites don't carry a project dir; the harness already trusts
/// `TAGENT_PROJECT_DIR` (and falls back to `current_dir()`) for similar
/// per-project paths, so we follow the same convention.
/// What: Returns `TAGENT_PROJECT_DIR` if set, else `std::env::current_dir()`,
...
mod tests {
    ...
    async fn append_usage_creates_file() {
        ...
        let path = dir.path().join(".trusty-agents/state/usage.jsonl");
