...
fn cache_in(base: &Path) -> GlobalSkillsCache {
    let skills = base.join(".trusty-agents").join("skills");
    GlobalSkillsCache {
...
async fn test_refresh_scans_project_skills() {
    ...
    let skills_dir = project.join(".trusty-agents").join("skills");
...
fn test_discovery_paths_project_first() {
    ...
    assert_eq!(paths[0].0, project.join(".trusty-agents/skills"));
...
async fn refresh_global_cache_is_noop_on_missing_sources() {
    ...
    // No `.trusty-agents/skills` exists under `project` → refresh must still succeed
