//! Configurable skill sources for `.trusty-agents/skill-sources.toml` (#172).
//!
...
//! full ordered list parsed from `.trusty-agents/skill-sources.toml`). Provides
...
/// repository whose contents are mirrored under `~/.trusty-agents/skills/cache/`.
///
...
/// Top-level deserialization shape for `.trusty-agents/skill-sources.toml`.
...
impl SkillSourceRegistry {
    /// Load `<project_root>/.trusty-agents/skill-sources.toml` or fall back to
    /// built-in defaults (project + user `.trusty-agents/skills`).
    ///
...
/// working unchanged.
/// What: Project-local `.trusty-agents/skills` (priority 10) + user-level
/// `~/.trusty-agents/skills` (priority 5).
fn default_sources() -> Vec<SkillSource> {
    vec![
        SkillSource {
            source_type: SkillSourceType::Local,
            path: Some(".trusty-agents/skills".to_string()),
            name: None,
            url: None,
...
/// Cache dir for a remote-git source: `~/.trusty-agents/skills/cache/<name>/`.
///
