...
fn skill_search_paths_order() {
    ...
    assert_eq!(
        paths[idx],
        PathBuf::from("/tmp/skills-home-test/.trusty-agents/skills")
    );
    assert_eq!(
        paths[idx + 1],
    ...
    assert_eq!(
        paths[idx + 2],
        PathBuf::from("/opt/trusty-agents/config/skills")
    );
...
// ── Integration-style tests against bundled .trusty-agents/skills/ ──────────
//
// Why: Confirms that the bundled `.md` skills under `.trusty-agents/skills/`
