...
impl SkillRegistry {
    ...
    ///
    /// Why: The global skills cache at `~/.trusty-agents/skills/files/` and
    ...
    /// What: Loads project-local `.trusty-agents/skills/` first (highest priority);
    /// then appends entries from `~/.trusty-agents/skills/files/` and
    /// `~/Projects/skillset-mcp` that do not already exist by name.
    /// Test: Write a skill in `~/.trusty-agents/skills/files/`; call with a project
    ...
    pub async fn load_with_global_cache(project_dir: &Path) -> anyhow::Result<Self> {
        ...
        let local_dir = project_dir.join(".trusty-agents").join("skills");
