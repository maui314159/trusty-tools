...
//! trusty-agents agents we need a small async client that handles the handshake,
...
/// What: Uses `$HOME/.trusty-agents/logs/<slug>-stderr.log` when HOME is set, or
/// `/tmp/trusty-agents-<slug>.log` as a fallback. Parent directories are created
/// best-effort.
...
pub(crate) fn plugin_stderr_stdio(binary: &str) -> std::process::Stdio {
    ...
    let was_new = std::fs::metadata(&path).is_err();
    {
            ...
            && !set.contains(&path)
        {
            ...
            if was_new {
                println!(
                    "trusty-agents: plugin '{slug}' stderr -> {} (created)",
                    path.display()
                );
            } else {
                debug!(
                    "trusty-agents: plugin '{slug}' stderr -> {}",
                    path.display()
                );
            }
        }
