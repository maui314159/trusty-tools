//! `ProjectRegistry` — the on-disk store backed by `~/.trusty-agents/projects.json`.
//!
...
/// Global project registry backed by `~/.trusty-agents/projects.json`.
///
...
pub struct ProjectRegistry {
    /// `~/.trusty-agents/projects.json`
    registry_path: PathBuf,
}

impl ProjectRegistry {
    /// Create a registry handle pointing at `~/.trusty-agents/projects.json`.
    ///
    ...
    pub fn new() -> Result<Self> {
        ...
        Ok(Self {
            registry_path: home.join(".trusty-agents").join("projects.json"),
        })
    }
    ...
    /// Register the trusty-agents self-project, flagging `is_self = true`. (#182)
    ///
    /// Why: CTRL detects its own source tree at startup so the user can
    /// dispatch development tasks against trusty-agents itself. We persist the
    ...
    /// `~/.trusty-agents/projects.json` without waiting for `deregister_missing`
