...
pub(super) async fn dispatch_subcommands(args: &[String]) -> Result<bool> {
    ...
    if args.len() > 1 {
        ...
        {
            eprintln!("trusty-agents: unknown subcommand '{candidate}'");
            eprintln!("  Did you mean '{suggestion}'?");
            eprintln!("  Run `tagent --help` for available commands.");
            std::process::exit(1);
        }
    }
    ...
    // launch trusty-agents REPL inside detached tmux session and render a
    ...
    // PM harness inspection: `trusty-agents inspect --task <text> [--dry-run]`.
    ...
    // #414: `trusty-agents plugins [list|status|check]` — report which optional
    ...
    // #403: `trusty-agents start|stop|status` — persistent background server lifecycle.
    //
    ...
    // #442: `trusty-agents dashboard` — launch the Tauri desktop GUI.
    //
