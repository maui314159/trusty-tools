...
/// observe report.
/// Test: `trusty-agents --workflow prescriptive --task-file t.md --out-dir /tmp/x`
/// loads `.trusty-agents/workflows/prescriptive.json` and runs each phase.
pub(super) async fn run_workflow(
    ...
    // project's `.trusty-agents/{agents,workflows}/` directories are missing.
    ...
    // `TAGENT_PROJECT_DIR` for every spawned agent.
    ...
    // `TAGENT_CONFIG_DIR`. Without this, sub-agents spawned with
    // `current_dir(out_dir)` would try to load `.trusty-agents/agents/<name>.toml`
    ...
    // sub-agent sees `TAGENT_PROJECT_DIR` and runs with CWD anchored at
    ...
    // #81/#115: Scan .trusty-agents/skills/ plus global paths and share the merged
    ...
    // ~/.trusty-agents/skills/index.json so the next run's ranking benefits from
    ...
    // ~/.trusty-agents/sessions/runs.jsonl so CTRL's `search_sessions` can grep
    ...
    // workflow result.
    {
        ...
        // user-visible result. The TAGENT_RUN_ID is the session id used
