...
//! trusty-agents entry point (PM orchestrator + sub-agent runner + direct/workflow modes).
//!
...
//!     loads `.trusty-agents/workflows/<name>.json` and runs each phase sequentially.
...
pub async fn run() -> Result<()> {
    ...
    // hint. The native trusty-agents `cli::did_you_mean` path (KNOWN_SUBCOMMANDS
...
/// Persist post-run skill effectiveness + usage to `~/.trusty-agents/skills/index.json`
/// (#171, #174).
///
...
/// inspecting `~/.trusty-agents/skills/index.json`. Behavior is unit-tested at the
...
mod main_tests {
    ...
    fn argv_as_task_text_strips_flags_and_joins() {
        let args: Vec<String> = vec!["trusty-agents", "write", "hello", "world"]
            .into_iter()
            .map(String::from)
            .collect();
    ...
    fn argv_as_task_text_ignores_long_flags() {
        let args: Vec<String> = vec!["trusty-agents", "--ctrl", "do", "thing"]
            .into_iter()
            .map(String::from)
            .collect();
    ...
    fn argv_as_task_text_empty_when_no_positional() {
        let args: Vec<String> = vec!["trusty-agents".to_string()];
...
// Bundled agent config directory — honors `TAGENT_CONFIG_DIR` with a
// CWD-relative `.trusty-agents/` fallback (#167).
//
...
// What: Returns `${TAGENT_CONFIG_DIR}` as-is if set (so search_paths
// appends `/agents`), else `./.trusty-agents`. Note: the repo's bundled config
// lives at `.trusty-agents/` now (formerly `config/`); runtime state is in
// `.trusty-agents/state/` (gitignored).
//
