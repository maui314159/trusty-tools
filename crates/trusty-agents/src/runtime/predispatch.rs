...
/// Handle the `trusty-agents /<slash> ...` one-shot passthrough.
///
...
/// Test: Indirectly via `cargo run -p trusty-agents` startup logs.
pub(super) async fn build_registries(cli: &Cli) {
    ...
    // #172: Load operator-configurable skill sources (.trusty-agents/skill-sources.toml)
    ...
    // (`~/.trusty-agents/skills/index.json` + content cache). Previously this only
        ...
        .is_err()
    {
        ...
        tracing::debug!("startup: defaulting TAGENT_SKILLS_PROJECT_LOCAL_ONLY=1 (interactive)");
    }
