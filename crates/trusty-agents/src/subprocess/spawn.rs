...
/// child via `TAGENT_OUT_DIR` so tools like `advance_workflow_phase` know
...
/// forwards the context: `assigned_file` → `TAGENT_ASSIGNED_FILE` child env;
/// `max_turns_override` → `TAGENT_MAX_TURNS` child env; `working_dir` →
/// `Command::current_dir`.
...
pub(super) struct SpawnConfig<'a> {
    ...
    /// `TAGENT_CODE_DIR` so the code-agent's WriteFileTool can root there
    ...
    /// #410: Optional project directory forwarded as `TAGENT_PROJECT_DIR`.
...
pub(super) async fn spawn_and_run_inner(cfg: SpawnConfig<'_>) -> Result<IpcMessage> {
    ...
    // them. session_id flows from TAGENT_RUN_ID (set in main.rs at startup),
    ...
    if let Some(dir) = out_dir {
        cmd.env("TAGENT_OUT_DIR", dir);
    }
    // #222: When set, TAGENT_CODE_DIR overrides TAGENT_OUT_DIR for
    // tools that write generated code (e.g. code-agent's WriteFileTool).
    if let Some(dir) = code_dir {
        cmd.env("TAGENT_CODE_DIR", dir);
    }
    ...
    if let Some(dir) = project_dir {
        cmd.env("TAGENT_PROJECT_DIR", dir);
    }
    if let Some(ctx) = ctx {
        ...
        if let Some(n) = ctx.max_turns_override {
            cmd.env("TAGENT_MAX_TURNS", n.to_string());
        }
        ...
        // `TAGENT_MODEL_<AGENT>` to subprocess children. The env var
        // resolution path (see `agents::mod::resolve_model`) treats
        // `TAGENT_MODEL_*` as the *highest* priority source, which means
        // any inherited override silently clobbers the child agent's own
        // TOML model (e.g. `bedrock/...` or `use_anthropic_direct = true`
        // configurations). The `TAGENT_MODEL_*` env vars are intended
...
/// `TAGENT_CONFIG_DIR` on the child process so it loads agent TOML from
