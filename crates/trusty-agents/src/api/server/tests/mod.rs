...
async fn tm_tell_returns_503_without_manager() {
    let body = Body::from(r#"{"project":"trusty-agents","message":"hi"}"#);
...
async fn connect_project_persists_to_registry() {
    ...
    // ~/.trusty-agents/projects.json that GET /api/projects reads from,
...
async fn get_project_config_falls_back_to_registry_when_toml_missing() {
    ...
    // `.trusty-agents/projects/<name>.toml`, so projects registered through
    ...
    // with any file in the repo's `.trusty-agents/projects/` directory.
