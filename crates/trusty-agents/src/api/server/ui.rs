...
//! run `trusty-agents --serve` and immediately get both the REST API and the web UI
...
//! Test: `cargo build && ./target/debug/trusty-agents --serve --port 7654 &` then
...
/// Test: After `cargo build && ./target/debug/trusty-agents --serve --port 7654 &`,
/// `curl -s http://localhost:7654/ | grep -c 'app'` should return > 0.
