...
/// `.trusty-agents/skills/personas/<name>.md`.
...
/// resolving under `.trusty-agents/skills/languages/`.
...
/// directory layout. Honors the `TAGENT_CONFIG_DIR` env var (set by the
...
/// is taken from `TAGENT_CONFIG_DIR` if set, otherwise the CWD-relative
/// `.trusty-agents/` fallback (matching `agents/mod.rs` behaviour).
...
///
/// Why: The agent loader uses `TAGENT_CONFIG_DIR/<name>.toml`. Skill files
/// live alongside agent files under the same root — for an installed binary
/// that means `TAGENT_CONFIG_DIR=/path/to/.trusty-agents/agents`, so the parent
