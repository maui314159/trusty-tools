...
///
/// Why: `chat_with_tools_gated` consults `TAGENT_AWS_PROFILE` /
/// `TAGENT_AWS_REGION` to construct the Bedrock client without taking new
...
// Why: Widened from `pub(crate)` to `pub` so the `trusty-agents` binary (now a
//      separate crate consuming `trusty-agents` as a library) can construct one
//      via `trusty_agents::agents::in_process_runner::BedrockEnvGuard::install`.
...
impl BedrockEnvGuard {
    pub fn install(profile: Option<&str>, region: Option<&str>) -> Self {
        ...
        unsafe {
            match profile {
                Some(p) => std::env::set_var("TAGENT_AWS_PROFILE", p),
                None => std::env::remove_var("TAGENT_AWS_PROFILE"),
            }
            match region {
                Some(r) => std::env::set_var("TAGENT_AWS_REGION", r),
                None => std::env::remove_var("TAGENT_AWS_REGION"),
            }
        }
        Self {
            prev_profile,
    ...
    fn drop(&mut self) {
        ...
        unsafe {
            match &self.prev_profile {
                Some(v) => std::env::set_var("TAGENT_AWS_PROFILE", v),
                None => std::env::remove_var("TAGENT_AWS_PROFILE"),
            }
            match &self.prev_region {
                Some(v) => std::env::set_var("TAGENT_AWS_REGION", v),
                None => std::env::remove_var("TAGENT_AWS_REGION"),
            }
        }
    }
