...
mod tests {
    ...
    #[test]
    fn fs_resolver_reads_trusty_agents_skills_md() {
        let tmp = tempdir();
        let skills_dir = tmp.join(".trusty-agents").join("skills");
    ...
    fn tempdir() -> PathBuf {
        let base =
            std::env::temp_dir().join(format!("trusty-agents-skill-test-{}", uuid::Uuid::new_v4()));
