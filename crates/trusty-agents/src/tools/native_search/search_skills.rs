...
//! walks `.trusty-agents/skills/` on disk; otherwise it returns a graceful payload.
...
/// absent, walks `.trusty-agents/skills/` on disk directly so the tool is still
...
pub struct SearchSkillsTool {
    ...
    /// Filesystem fallback root — defaults to `.trusty-agents/skills` under CWD.
    skills_dir: PathBuf,
}

impl SearchSkillsTool {
    /// Construct with default `.trusty-agents/skills` fallback and no resolver.
    pub fn new() -> Self {
        Self {
            resolver: None,
            skills_dir: PathBuf::from(".trusty-agents").join("skills"),
        }
    }
