...
fn sample_config() -> ProjectConfig {
    ProjectConfig {
        project: ProjectMeta {
            name: "trusty-agents".to_string(),
            path: PathBuf::from("/Users/masa/Projects/trusty-agents"),
...
fn test_roundtrip_with_harnesses() {
    ...
    let loaded = store.load("trusty-agents").unwrap().unwrap();
...
fn test_list_skips_non_toml_and_invalid() {
    ...
    assert_eq!(configs[0].project.name, "trusty-agents");
}
...
fn test_delete_is_idempotent() {
    ...
    let store = ProjectConfigStore::open(dir.path()).unwrap();
    store.save(&sample_config()).unwrap();
    store.delete("trusty-agents").unwrap();
    store.delete("trusty-agents").unwrap(); // second call is a no-op
    assert!(store.load("trusty-agents").unwrap().is_none());
}
...
fn test_find_by_path_returns_existing() {
    ...
    let found = store
        .find_by_path(Path::new("/Users/masa/Projects/trusty-agents"))
        .unwrap();
    assert!(found.is_some());
    assert_eq!(found.unwrap().project.name, "trusty-agents");
}
...
fn test_find_or_create_returns_existing_unchanged() {
    ...
    let cfg = store
        .find_or_create(
            Path::new("/Users/masa/Projects/trusty-agents"),
            // Different adapter id — must NOT mutate the existing config.
            "shell",
            Some("ignored-name-override"),
        )
        .unwrap();
    assert_eq!(cfg.project.name, "trusty-agents");
...
fn test_default_startup_command_for_known_adapters() {
    ...
    assert_eq!(default_startup_command_for("trusty-agents"), "om");
}
