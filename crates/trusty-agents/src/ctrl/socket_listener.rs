//! Per-project Unix-socket listener that lets the second `trusty-agents` invocation
...
///
/// Why: After binding the controller socket, every subsequent `trusty-agents`
...
/// the second `trusty-agents` invocation just becomes a thin client. Streaming
...
/// `trusty-agents "say hi"` in terminal B, observe streamed output.
///
