...
/// the project's `.trusty-agents/agents/` directory and failed loudly when it
/// wasn't there. This helper centralizes the priority order and keeps the
/// REPL launchable even with zero on-disk config.
/// What: Tries, in order:
///   1. `{project_path}/.trusty-agents/agents/pm.toml` (connected mode)
///   2. `~/.trusty-agents/agents/ctrl.toml` (user-level standalone)
///   3. `{project_path}/.trusty-agents/agents/ctrl.toml` (project-level ctrl)
...
pub(crate) async fn resolve_agent_config(
    ...
    if let Some(home) = dirs::home_dir() {
        let user_ctrl = home.join(".trusty-agents").join("agents").join("ctrl.toml");
...
/// its own repo (`detect_self_project()` succeeds and points at trusty-agents),
/// the project's `.trusty-agents/agents/pm.toml` exists and shadowed ctrl.toml,
...
/// Order:
///   1. `{project_path}/.trusty-agents/agents/ctrl.toml`
///   2. `~/.trusty-agents/agents/ctrl.toml`
///   3. `{project_path}/.trusty-agents/agents/pm.toml` (last-resort)
