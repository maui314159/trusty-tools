...
//!
//! Why: Lets the user / LLM manage `~/.trusty-agents/projects.json` from the REPL
...
/// `list_projects()` — dump active entries from ~/.trusty-agents/projects.json.
...
/// `add_project(path)` — register a project in `~/.trusty-agents/projects.json`.
/// (#202)
///
...
impl ToolExecutor for AddProjectTool {
    ...
    fn schema(&self) -> Value {
        json!({
            ...
            "function": {
                "name": "add_project",
                "description": "Register a project directory in ~/.trusty-agents/projects.json so it appears in list_projects.",
                "parameters": {
                    "type": "object",
                    "properties": {
...
/// `remove_project(path)` — drop an entry from `~/.trusty-agents/projects.json`.
/// (#202)
///
...
impl ToolExecutor for RemoveProjectTool {
    ...
    fn schema(&self) -> Value {
        json!({
            ...
            "function": {
                "name": "remove_project",
                "description": "Remove a project entry from ~/.trusty-agents/projects.json. Does not stop running PM sessions.",
                "parameters": {
                    "type": "object",
                    "properties": {
