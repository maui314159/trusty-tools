...
async fn run_ctrl_inner(with_stdin: bool, ready_tx: Option<oneshot::Sender<()>>) -> Result<()> {
    ...
    .is_err()
    {
        ...
        tracing::debug!("CTRL: defaulting TAGENT_SKILLS_PROJECT_LOCAL_ONLY=1");
    }
...

    {
        ...
        tokio::spawn(async move {
            ...
            if let Some(idx) = idx {
                ...
                tracing::info!(
                    "[trusty-agents] Docs index: {n} documents indexed from {}",
                    docs_root.display()
                );
            }
        });
...

    {
        let project_root = ctrl
            ...
            .unwrap_or_else(|| std::env::current_dir().unwrap_or_else(|_| PathBuf::from(".")));
        tokio::spawn(async move {
            let omd = project_root.join(".trusty-agents").join("state");
    ...
    match CtrlSocket::bind_singleton_default(&sock_path).await {
        Ok(BindOutcome::Bound(listener)) => {
            tracing::info!(
                "[trusty-agents] controller socket listening at {}",
                sock_path.display()
            );
