...
impl ProgressReporter {
    ...
    pub fn format_phase_start(&self, phase: &str) -> String {
        ...
        format!("[trusty-agents] {arrow} {name} starting…")
    }
    ...
    pub fn format_phase_done(
        ...
        match note {
            Some(n) if !n.is_empty() => {
                format!("[trusty-agents] {check} {name} done  {stats} — {n}")
            }
            _ => format!("[trusty-agents] {check} {name} done  {stats}"),
        }
    }
    ...
    pub fn phase_failed(&self, phase: &str, elapsed: Duration, error: &str) {
        ...
        eprintln!("\r[trusty-agents] {cross} {name} failed {stats} — {short}");
    }
    ...
    pub fn wave_start(&self, wave_index: usize, wave_total: usize, file_count: usize) {
        ...
        eprintln!(
            "\r[trusty-agents] {arrow} {name} wave {wave_index}/{wave_total}  ({file_count} file{plural})",
    ...
    pub fn wave_done(&self, wave_index: usize, wave_total: usize, elapsed: Duration) {
        ...
        eprintln!("\r[trusty-agents] {check} {name} wave {wave_index}/{wave_total} done {stats}");
    }
    ...
    pub fn workflow_done(&self, total_elapsed: Duration, total_cost: f32) {
        ...
        eprintln!("\r[trusty-agents] {check} {name} complete  {stats}");
    }
...
mod tests {
    ...
    fn progress_reporter_formats_phase_start() {
        ...
        assert!(s.starts_with("[trusty-agents]"), "got: {s}");
