...
//! trait to the `TrustyAgentsRepl` slash-command surface.
//!
...
//! in all REPL state, so the bridge owns an `Arc<Mutex<TrustyAgentsRepl>>` and
...
/// `Arc<Mutex<TrustyAgentsRepl>>`.
pub(crate) struct ReplBridge {
    pub(crate) repl: Arc<tokio::sync::Mutex<TrustyAgentsRepl>>,
}
        ...
        if trimmed.starts_with('/') {
            ...
            if cmd_only == "/update" {
                match crate::update::check_for_update().await {
                    Some(info) => {
                        let msg = format!(
                            "Upgrading from v{} to v{}…\nRunning: cargo install --git https://github.com/bobmatnyc/trusty-agents trusty-agents",
                        ...
                        match tokio::process::Command::new("cargo")
                            .args([
                                "install",
                                "--git",
                                "https://github.com/bobmatnyc/trusty-agents",
                                "trusty-agents",
                            ])
                            .status()
                            .await
                        {
                            Ok(status) if status.success() => {
                                let done = format!(
                                    "✓ Upgraded to v{}. Restart trusty-agents to use the new version.",
                                    info.latest_version
                                );
                            ...
                            Ok(status) => {
                                let _ = tx.send(tui::ReplEvent::StatusMessage(format!(
                                    "✗ Upgrade failed (exit {}). Try: cargo install --git https://github.com/bobmatnyc/trusty-agents trusty-agents",
                                    status
                                )));
                            }
                            Err(e) => {
