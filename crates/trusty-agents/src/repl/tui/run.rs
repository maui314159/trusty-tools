...
/// table, persona switching, ctrl socket forwarding). The outer `TrustyAgentsRepl`
...
pub struct ReplStartup {
    ...
    /// Project dir whose `.trusty-agents/state/usage.json` we'll read/write for
    ...
    // message in the chat (and the user can run `/update` to upgrade).
    {
        ...
        tokio::spawn(async move {
            if let Some(info) = crate::update::check_for_update().await {
                let msg = format!(
                    "Update available: trusty-agents v{} (you have v{}) — run /update to upgrade",
                    info.latest_version,
