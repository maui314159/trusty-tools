//! `TrustyAgentsRepl::run` — the REPL entry point, split from `mod.rs` for the
//! 500-line file cap (#357). The remaining `TrustyAgentsRepl` methods live in
...
use super::{TrustyAgentsRepl, banner, statusline, strip_vendor_prefix_for_display, tui};
...
impl TrustyAgentsRepl {
    ...
    pub async fn run(&mut self) -> Result<()> {
        ...
        let placeholder = TrustyAgentsRepl::new(self.user.clone())?;
        ...
        initial_chat_messages.push(format!(
            "trusty-agents v{} — type /help for commands",
        ...
        let restored = std::mem::replace(&mut *guard, TrustyAgentsRepl::new(None)?);
        *self = restored;
