//! Ratatui-based interactive REPL — the primary user interface for trusty-agents.
//!
...
//! What: `TrustyAgentsRepl::run()` enters alt-screen via `tui::run_tui` and routes
...
pub struct TrustyAgentsRepl {
    pub(crate) user: Option<UserProfile>,
    pub(crate) project_name: String,
    pub(crate) socket_path: PathBuf,
    pub(crate) history_path: PathBuf,
    /// Resolved project directory — drives all `.trusty-agents/*` lookups and
    ...
    /// running `trusty-agents --serve` daemon (#343). User messages are
...
impl TrustyAgentsRepl {
    ...
    pub fn new(user: Option<UserProfile>) -> Result<Self> {
        ...
        let history_path = home.join(".trusty-agents").join("repl_history.txt");
        if let Some(parent) = history_path.parent() {
            std::fs::create_dir_all(parent).ok();
        }

        // Why bug fix (#statusline-shows-sonnet): default project_dir to the
        // bundled trusty-agents project root when the REPL is launched standalone
        // (i.e. cwd has no `.trusty-agents/`). Falls through `TAGENT_PROJECT_DIR`
        ...
        let project_dir = std::env::current_dir()
            .ok()
            .filter(|d| d.join(".trusty-agents").join("agents").is_dir())
            .or_else(crate::ctrl::detect_self_project)
            .unwrap_or_else(|| std::env::current_dir().unwrap_or_else(|_| PathBuf::from(".")));
        let agents_dir = project_dir.join(".trusty-agents").join("agents");
        let skills_dir = project_dir.join(".trusty-agents").join("skills");
        ...
        // tmux manager rooted at the project's `.trusty-agents/state/` directory.
    ...
    ///
    /// Why (#343): When `trusty-agents` is launched and a daemonized `--serve`
    ...
    /// Test: Manual via `trusty-agents --service start && trusty-agents`.
    ...
    /// launched from a directory without `.trusty-agents/` but had a user-level
    /// `~/.trusty-agents/agents/ctrl.toml` with sonnet, the project path missed,
    ...
    pub(crate) fn agent_toml_search_paths(&self) -> Vec<PathBuf> {
        ...
        if self.project_name == "pm" {
            ...
            if let Some(home) = dirs::home_dir() {
                paths.push(home.join(".trusty-agents").join("agents").join("ctrl.toml"));
            }
            paths.push(
                self.project_dir
                    .join(".trusty-agents")
...
impl std::fmt::Debug for TrustyAgentsRepl {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("TrustyAgentsRepl")
