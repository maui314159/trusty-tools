...
`trusty-agents-local` provides local process execution capabilities to the MPM orchestrator:
...
Implements `trusty-agents-common::Agent` trait:
...
```rust
use trusty_agents_agent_api::{AgentRequest, AgentContext, Constraints};
use trusty_agents_local::LocalExecutor;
...
```rust
use trusty_agents_local::CommandExecutor;
...
### With trusty-agents Orchestrator
...
- `crates/trusty-agents-common/README.md` for agent trait definition
- `crates/trusty-agents/README.md` for orchestrator
- Security best practices in `docs/trusty-agents-local/`
